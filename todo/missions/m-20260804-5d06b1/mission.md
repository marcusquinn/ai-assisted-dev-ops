...
- The current 14 primary aidevops agents and the `aidevops` framework specialist are generated from canonical discovery, not a duplicated hard-coded registry.
...
**Status:** complete
**Estimate:** 25.5h
**Validation:** The merged non-mutating runtime, canonical roster, Buzz and Matrix adapters, normalized Matrix ingress/session boundary, and restricted OpenCode overlay pass their integrated contract, security, compatibility, launcher, installed-runtime, and 595-test plugin suites without provider writes or persistent user-config mutation.
...
| 2.4 | Refactor the existing Matrix integration behind the same provider/event/authority contract without behavior regression `[depends:F2.1]` | t18207 | complete | 6h | marcusquinn | #29670 |
| 2.5 | Generate restricted OpenCode launch overlays for canonical agent selection, workload variant, interface context, and read-only conversational permissions `[depends:F2.2]` | t18208 | complete | 5h | marcusquinn | #29673 |
...
| 23 | 2026-08-09 | Add Private AI as the fourteenth canonical primary and fifteenth Buzz member with a reviewed shared-compute exception | The owner requested an immediately available privacy-first investigator while retaining fourteen existing full-interactive members; explicit routing and warnings prevent shared compute from being misrepresented as local execution | Defer all Private AI presence to feature 5.6; move every member to shared compute; claim relay routing is on-device |
...
| 2026-08-07T02:24:18Z | Milestone 2 restricted OpenCode overlay checkpoint | Published Feature 2.5 in PR #29673 with deterministic schema-backed descriptors, roster/source binding, bounded context injection, workload-tier routing, fail-closed OpenCode isolation, a fixed-argv ACP launcher, security and compatibility coverage, and operator documentation. The original head passed broad, runtime, and Qlty gates, but the PR was later closed unmerged after changes-requested review; this checkpoint is not completion evidence. |
| 2026-08-07T04:10:06Z | Milestone 2 restricted OpenCode review repair verified | Accepted all six security and bookkeeping findings and hardened current-roster/selected-agent binding, private runtime and bootstrap attestation, missing-overlay failure, registered project/worktree roots, credential-safe read/search paths, and the conversation-only plugin hook surface in repair commit `dd159b000`. Focused overlay, 14-case plugin profile, 16-case launcher, and installed OpenCode 1.18.9 effective-config/ACP startup canaries pass. Feature 2.5 remains in review until the repair is replayed onto, reviewed on, and merged through PR #29673. |
| 2026-08-07T04:47:10Z | Milestone 2 restricted OpenCode repair replayed | Replayed all accepted repair commits onto PR #29673's preserved head, integrated current `origin/main` without force-push, and verified code checkpoint `72990d372`. The full 565-test plugin suite, 12 broad team-interface suites, 16-case launcher, runtime dependency check, installed OpenCode 1.18.9 ACP canary, changed-file lint, Qlty 49-to-49 regression parity, and zero-smell new-file gate pass. Feature 2.5 still requires fast-forward publication, PR reopen, exact-head remote review/CI, and merge. |
| 2026-08-07T05:42:13Z | Milestone 2 restricted OpenCode overlays completed | PR #29673 merged as `670d77b074928d70ef07e62055c8f434c7258db1` after accepted security repairs, exact-head review, runtime evidence, and required CI passed. The implementation is complete even though child #29647 retains its earlier terminal `NOT_PLANNED` lifecycle reason. |
| 2026-08-07T06:40:53Z | Milestone 2 post-merge verification repaired | PR #29689 restored the Qlty baseline to 49/49 without raising the threshold. PR #29687 then repaired isolation-sensitive post-merge verification tests and established the merged integration baseline at `0c817c462fc87d583c89a6ab768807daab5892df`. |
| 2026-08-07T06:54:21Z | Milestone 2 integrated validation completed | Verified every native #29541 child terminal and reran merged-main runtime/core/reconciliation, roster/discovery/tier, Buzz, Matrix adapter/ingress/session, restricted OpenCode overlay/profile/launcher/runtime, shell, and plugin gates. The full plugin suite passes 595/595; no provider write, persistent user-config mutation, deployment, publication, or release occurred. |
| 2026-08-09T04:55:34Z | Buzz identity-preserving update requirement confirmed live | Deleting and recreating an imported agent left its existing DM and threads bound to the removed identity; the replacement initialized successfully but discovered zero channels until a new DM established membership, after which it replied immediately. Future managed updates must reconcile records in place, preserve agent identity and channel lineage, and treat delete/recreate as destructive migration rather than routine update. This evidence reinforces the existing stable external ID, revision/CAS, and supported apply-API gate; no new task or issue was created. |
| 2026-08-09T05:19:26Z | Owner-reviewed interactive runtime and Private AI leaf implemented | Task t18222 / issue #29831 now carries an immutable full-interactive runtime with sanitized OpenCode config, exact plugin/SDK/Zod packaging, and a real ACP prompt canary. Canonical discovery includes Private AI as the fourteenth primary; the Buzz snapshot contains fifteen owner-only members, with only Private AI using `buzz-agent`/`relay-mesh`/`auto`. Full feature 5.6 onboarding/reconciliation remains pending. |
| 2026-08-09T17:02:40Z | Owner-reviewed Buzz runtime released and deployed | PR #29834 merged and signed release `v3.32.242` reached GitHub, npm, and Homebrew with a terminal publication receipt. Exact-tag OpenCode 1.18.9 ACP prompt verification passed against immutable schema-v2 anchor `v2-96d48a0453921853-00bb29b2b0f650ad`; the live custom harness moved to that anchor after a private rollback copy while every existing Buzz identity, DM, thread, and team record remained untouched. Deployment also produced a regression test for resolving framework dependencies from deployed `agents/node_modules`. |
...
| 2026-08-06T06:16:57Z | 2 / t18201 | Pulse closed #29541 after its bodyless canonical snapshot exposed only three terminal native children; the live issue still contained unchecked acceptance criteria and two deliberately unfiled features | Mission rows, t18201 close contract, live issue body, native child graph, closure comment, and exact reconciler/cache paths were preserved | Satisfied: #29650 repaired the systemic failure, #29646/#29647 reached terminal state with merged implementations, and integrated validation passed | Merge the final t18201 parent-close PR, synchronize the completed contract, and verify #29541 closes completed | resolved |
