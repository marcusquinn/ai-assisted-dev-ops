...
def _health(connection: Connection, record: dict[str, Any], now: int) -> str:
    ...
    if not connection.enabled:
        ...
        health = "rate-reset"
    elif isinstance(record.get("consecutive_failures", 0), int) and record.get("consecutive_failures", 0) >= connection.alert_after_failures:
...
def plan(connections: list[Connection], state: dict[str, Any], now: int) -> list[dict[str, Any]]:
    ...
    for connection in connections:
        ...
        last_success = record.get("last_success", 0)
        result.append({
            "connection_id": connection.connection_id, "connector_id": connection.connector_id,
            "due": boundary is not None and boundary <= now,
            "freshness_lag_seconds": now - last_success if last_success else None,
            "health": health, "missed_sla": health in ("stale", "terminal-failure"),
            "mode": connection.mode, "next_due": boundary,
        })
    return result
