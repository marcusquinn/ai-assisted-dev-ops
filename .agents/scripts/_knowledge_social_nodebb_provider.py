...
from _knowledge_social_nodebb_routes import page
...
def _profile_prefix(profile: str) -> str:
    if PROFILE_NAME.fullmatch(profile) is None:
        raise NodeBBReadProviderError("NodeBB profile name is invalid")
    return f"NODEBB_{profile.upper()}"
...
def _profile_value(prefix: str, suffix: str, field: str, limit: int) -> str:
    ...
    if not value or "\x00" in value or len(value.encode()) > limit:
        raise NodeBBReadProviderError(f"NodeBB profile {field} is missing")
    return value
...
def _identity(config: ProfileConfig, opener: Opener, expected_id: str) -> dict[str, Any]:
    result = api(config, opener, "/api/self", {})
...
def _verify_page_account(
    ...
    if (
        ...
        or data.get("userslug") != request.userslug
...
def _dispatch(
    ...
    if action != "page":
        raise NodeBBReadProviderError("NodeBB read action is unsupported")
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except (NodeBBReadProviderError, NodeBBAdapterError) as error:
        print(f"ERROR: {error}", file=sys.stderr)
        return 1
    except Exception:  # noqa: BLE001 - intentionally redact provider internals
        print("ERROR: NodeBB read provider request failed", file=sys.stderr)
        return 1
