...
def parse_telegram_updates(request: TelegramRequest) -> ParsedTelegramBatch:
    ...
    for raw_update in require_list(root.get("updates"), "updates"):
        ...
        if update_type in MESSAGE_UPDATES:
            ...
            poll_id = stable_id(value.get("id"), "poll ID")
            objects.append(
                {
                    "object_type": "poll",
                    "remote_id": f"poll:{poll_id}",
                    "account_remote_id": None,
                    "text": value.get("question") if isinstance(value.get("question"), str) else None,
                    "created_at": None,
                    "observed_at": observed_at,
                    "evidence_class": "observed",
                    "provider_json": {"source": "telegram_bot_api_update", "is_closed": value.get("is_closed")},
                }
            )
            ...
            activities.append(
                {
                    ...
                    "observed_at": observed_at,
                    "state": "observed",
                    "provider_json": {"source": "telegram_bot_api_update", "update_type": update_type},
                }
            )
    ...
    archive = {
        ...
        "policy": {
            ...
            "network_requests": 0,
            "owner_id": owner,
            "privacy_mode": root.get("privacy_mode") if root.get("privacy_mode") in ("enabled", "disabled", "unknown") else "unknown",
