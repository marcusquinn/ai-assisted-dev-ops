...
inventory_untracked_files() {
	...
	python3 - "$visible_path" "$ignore_dirs_path" <<'PY'
import pathlib
import sys
...
PY
	...
	python3 - "$combined_path" "$capture_dir/$UNTRACKED_LIST_NAME" <<'PY'
import pathlib
import sys
...
for raw in source:
    ...
    if path.is_absolute() or ".." in path.parts:
        raise SystemExit("unsafe untracked inventory")
    paths.add(raw)
...
hash_untracked_payload() {
	...
	local nul_path="$2"
	python3 - "$payload_root" "$nul_path" "$MAX_UNTRACKED_FILES" "$MAX_UNTRACKED_BYTES" <<'PY'
import hashlib
import os
import pathlib
import stat
...
copy_untracked_files() {
	...
	python3 - "$repo_path" "$backup_dir/untracked" "$nul_path" "$MAX_UNTRACKED_FILES" "$MAX_UNTRACKED_BYTES" <<'PY'
import os
import pathlib
import shutil
import stat
...
restore_preserved_untracked() {
	local repo_path="$1"
	local backup_dir="$2"
	python3 - "$repo_path" "$backup_dir/untracked" "$backup_dir/$UNTRACKED_LIST_NAME" <<'PY'
import os
import pathlib
import shutil
import stat
...
for raw in paths:
    ...
    relative = pathlib.Path(os.fsdecode(raw))
    if relative.is_absolute() or ".." in relative.parts:
        raise SystemExit("unsafe untracked restore inventory")
