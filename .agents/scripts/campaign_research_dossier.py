...
def build_dossier(campaign_id: str, intake: dict[str, Any], source_paths: list[Any], now: dt.datetime) -> dict[str, Any]:
    ...
    dossier = _assemble_dossier(campaign_id, intake, ledgers, observations, grouped, now)
...
def _assemble_dossier(campaign_id: str, intake: dict[str, Any], ledgers: list[dict[str, Any]], observations: list[dict[str, Any]], grouped: dict[str, list[dict[str, Any]]], now: dt.datetime) -> dict[str, Any]:
    ...
    audiences = [{"label": required_text(audience.get("segment"), "intake audience", 240), "summary": "Research question seeded from the approved campaign intake.", "confidence": "medium", "evidence_source_ids": ["campaign-intake"]} for audience in intake["audiences"] if isinstance(audience, dict)]
    snapshot = {"campaign_id": campaign_id, "intake": intake, "ledger": ledgers, "observations": observations}
    return {"schema_version": SCHEMA_VERSION, "campaign_id": campaign_id, "semantic_snapshot_sha256": hashlib.sha256(canonical_bytes(snapshot)).hexdigest(), "generated_at": now.replace(microsecond=0).isoformat().replace("+00:00", "Z"), "coverage": {"status": coverage, "source_counts": counts}, "audiences": audiences + grouped["audience"], "buying_roles": roles, "competitors": grouped["competitor"], "creators": grouped["creator"], "trends": grouped["trend"], "channel_fit": grouped["channel_fit"], "opportunities": grouped["opportunity"], "contradictions": grouped["contradiction"], "provenance_ledger": ledgers}
