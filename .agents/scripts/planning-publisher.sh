...
_planning_publish_write_receipt() {
	...
	(
		umask 077
		{
			printf 'format=aidevops-planning-publication-v1\n'
...
planning_verify_publication_receipt() {
	...
	local source_head="" published_commit="" published_parent="" publication_id=""
	...
	[[ "$format" == "aidevops-planning-publication-v1" && "$repository_id" == "$expected_repository_id" && \
	...
	[[ "$source_head" =~ ^[0-9a-fA-F]{40,64}$ && "$published_commit" =~ ^[0-9a-fA-F]{40,64}$ && \
		"$published_parent" =~ ^[0-9a-fA-F]{40,64}$ && "$publication_id" =~ ^[0-9a-fA-F]{40,64}$ ]] || return 1
...
_planning_publish_record_noop_receipt() {
	...
	[[ "${AIDEVOPS_PLANNING_WRITE_RECEIPT:-false}" == "true" ]] || return 0
	_planning_publish_commit_has_publication_id "$repo_path" "$latest_sha" "$publication_id" || return 0
	...
	_planning_publish_write_receipt "$repo_path" "$remote_name" "$branch_name" "$source_head" \
		"$latest_sha" "$parent_sha" "$publication_id" "$snapshot_file" || return 1
	return 0
}
...
_planning_publish_record_pushed_receipt() {
	...
	if ! _planning_publish_write_receipt "$repo_path" "$remote_name" "$branch_name" "$source_head" \
		"$candidate_sha" "$parent_sha" "$publication_id" "$snapshot_file"; then
