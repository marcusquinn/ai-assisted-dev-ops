...
Custom profiles remain untouched. Aidevops-managed profiles are reconciled
conservatively: stale managed entries and duplicate managed entries are removed,
while registered missing paths, custom entries, and valid orphan profiles remain.
"""
...
def get_registered_profile_paths(repos_json_path: str) -> set[str]:
    """Return all explicit profile paths, including registered missing paths.

    ``get_repos`` deliberately filters unavailable paths before generating new
    profiles. Reconciliation needs the complementary set so a temporarily
    unavailable but explicitly registered canonical repository is never removed.
    """
    with open(repos_json_path) as handle:
        repos = json.load(handle).get("initialized_repos", [])
    return {
        os.path.normcase(os.path.realpath(os.path.expanduser(path)))
        for repo in repos
        if isinstance(repo, dict) and isinstance((path := repo.get("path")), str) and path
    }


def _profile_blocks(config_text: str) -> list[tuple[str, str]]:
    """Return ``(prefix, block)`` pairs without reformatting Tabby's YAML."""
    ...
    while index < len(lines):
        ...
        while index < len(lines) and not lines[index].startswith("  - name:"):
            if lines[index] and not lines[index][0].isspace():
                break
            index += 1
        blocks.append(("".join(prefix), "".join(lines[start:index])))
        prefix = []
    if prefix:
        blocks.append(("".join(prefix), ""))
    return blocks


def _profile_cwd(profile: str) -> str:
    """Return a profile cwd without accepting unrelated YAML text."""
    paths = extract_existing_cwds(profile)
    return next(iter(paths), "") if len(paths) == 1 else ""


def _is_managed_profile(profile: str) -> bool:
    """Identify only profiles owned by aidevops, never custom OpenCode profiles."""
    return TABBY_OPENCODE_LAUNCH in profile


def reconcile_managed_profiles(
    config_text: str, registered_paths: set[str]
) -> tuple[str, dict[str, int]]:
    """Remove only stale or duplicate aidevops-managed Tabby profiles.

    A managed profile is removable when its normalized cwd is unavailable and
    absent from repos.json. For duplicate managed profiles, retain the first
    matching profile. Profiles not generated by aidevops are byte-preserved.
    """
    retained: list[str] = []
    pending_prefix = ""
    seen_managed_cwds: set[str] = set()
    removed_stale = 0
    removed_duplicate = 0
    for prefix, profile in _profile_blocks(config_text):
        if not profile or not _is_managed_profile(profile):
            retained.append(pending_prefix + prefix + profile)
            pending_prefix = ""
            continue
        cwd = _profile_cwd(profile)
        normalized = os.path.normcase(os.path.realpath(os.path.expanduser(cwd))) if cwd else ""
        if normalized and normalized in seen_managed_cwds:
            removed_duplicate += 1
            pending_prefix += prefix
            continue
        if normalized and not os.path.isdir(cwd) and normalized not in registered_paths:
            removed_stale += 1
            pending_prefix += prefix
            continue
        if normalized:
            seen_managed_cwds.add(normalized)
        retained.append(pending_prefix + prefix + profile)
        pending_prefix = ""
    if pending_prefix:
        retained.append(pending_prefix)
    return "".join(retained), {"stale": removed_stale, "duplicates": removed_duplicate}


def show_status(
    repos: list[dict], existing_cwds: set[str], reconciliation: dict[str, int]
) -> None:
    ...
    removed = reconciliation["stale"] + reconciliation["duplicates"]
    print(
        "Reconciliation: "
        f"{reconciliation['stale']} stale, {reconciliation['duplicates']} duplicate "
        f"managed profile(s) would be removed ({removed} total)."
    )
    if needs_profile > 0:
...
def sync_profiles(args: argparse.Namespace) -> None:
    ...
    config_text, reconciliation = reconcile_managed_profiles(
        config_text, get_registered_profile_paths(args.repos_json)
    )
    ...
    removed_count = reconciliation["stale"] + reconciliation["duplicates"]
    if not new_profiles:
        if repaired_count or removed_count:
            ...
            if removed_count:
                print(
                    "Removed "
                    f"{reconciliation['stale']} stale and {reconciliation['duplicates']} "
                    "duplicate managed Tabby profile(s)."
                )
            if repaired_count:
...
def main() -> None:
    ...
    repos = get_profile_targets(args.repos_json)
    config_text = load_yaml_simple(args.tabby_config)
    reconciled_text, reconciliation = reconcile_managed_profiles(
        config_text, get_registered_profile_paths(args.repos_json)
    )
    existing_cwds = extract_existing_cwds(reconciled_text)
