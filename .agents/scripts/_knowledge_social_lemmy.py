...
STREAMS = {
    "authored_posts": StreamSpec("post", "content_author"),
    "authored_comments": StreamSpec("comment", "content_author"),
    "saved_posts": StreamSpec("post", "selected_account"),
    "saved_comments": StreamSpec("comment", "selected_account"),
    "liked_posts": StreamSpec("post", "selected_account"),
    "liked_comments": StreamSpec("comment", "selected_account"),
    "notifications": StreamSpec("notification", "selected_account"),
    "replies": StreamSpec("reply", "selected_account"),
    "mentions": StreamSpec("mention", "selected_account"),
    "subscriptions": StreamSpec("community", "selected_account"),
    "multicommunities": StreamSpec("multicommunity", "selected_account"),
}
...
def _encode_cursor(family: str, version: str, position: int, token: str) -> str:
...
def _decode_cursor(cursor: str, family: str, version: str) -> tuple[int, str]:
    ...
    if not isinstance(parsed, dict) or set(parsed) != {
        "api_family", "exact_version", "page_value", "position",
    }:
...
def page_request(stream: str, account: dict[str, Any], state: CursorState, limit: int) -> PageRequest:
    ...
    position, page_cursor = (1, None)
    if state.cursor:
        position, page_cursor = _decode_cursor(state.cursor, family, version)
    ...
    return PageRequest(
        ...
        position,
        page_cursor,
        _text(state.watermark, "watermark", optional=True),
        limit,
    )
...
def page_checkpoint(
    ...
    watermark = _text(meta.get("watermark"), "watermark", optional=True)
    if watermark is None:
        watermark = state.watermark
    cursor = (
        _encode_cursor(request.api_family, request.exact_version, request.position + 1, token)
