...
if _full_loop_validate_release_candidates test/repo "$source_json" >/dev/null 2>&1; then
	printf 'FAIL release preparation accepted prior release:not-requested evidence\n'
	exit 1
fi
_full_loop_validate_release_candidates test/repo "$source_json" \
	"$_FULL_LOOP_RELEASE_RECONCILE_PUBLISHED" v1.2.3 "$expected_tag_commit" || {
	printf 'FAIL published aggregate reconciliation rejected prior release:not-requested evidence\n'
	exit 1
}
...
printf 'PASS only post-publication reconciliation may preserve release:not-requested evidence\n'

printf 'published\n' >"${TEST_ROOT}/receipts/test_repo-91.status"
if _full_loop_validate_release_candidates test/repo "$source_json" \
	"$_FULL_LOOP_RELEASE_RECONCILE_PUBLISHED" v1.2.3 "$expected_tag_commit" >/dev/null 2>&1; then
...
_full_loop_persist_release_success test/repo "${TEST_ROOT}/release" "$source_json" 99 "$sha99" \
	"$_FULL_LOOP_RELEASE_RECONCILE_PUBLISHED" || {
...
jq -e --arg source_merge "$sha90" --arg aggregate_merge "$sha99" \
	...
	.evidence_type == "published-aggregate-terminal-receipt-conflict" and .status == "receipt-conflict"
	and .preserved_receipt_status == "not-requested" and .pr_number == 90
	and .source_merge == $source_merge and .aggregate_pr == 99
	and .aggregate_merge == $aggregate_merge and .release_tag == "v1.2.3"
	and .release_commit == $release_commit and (.terminal_cleanup_evidence | not)
' "${TEST_ROOT}/receipts/test_repo-90.receipt-conflict.json" >/dev/null || {
	printf 'FAIL published aggregate conflict evidence was incomplete\n'
	exit 1
}

printf 'PASS published aggregate reconciliation preserves no-release receipts with conflict evidence\n'
exit 0
