...
assert_contains ".github/workflows/linked-issue-check.yml" 'const hasTaskPrefix = .*t\\d.*GH#\\d' "linked issue recognizes both tNNN and GH#NNN prefixes"
assert_contains ".github/workflows/linked-issue-check.yml" "const trustedRoles = \\['OWNER', 'MEMBER', 'COLLABORATOR'\\]" "linked issue preserves trusted author controls"
assert_contains ".github/workflows/linked-issue-check.yml" "pr.user.type === 'Bot'" "linked issue preserves bot control"
assert_contains ".github/workflows/linked-issue-check.yml" 'trustedRoles\.includes\(pr\.author_association\) && hasTaskPrefix' "linked issue title exemption requires trusted author association"
assert_contains ".github/workflows/linked-issue-check.yml" '#aidevops:trust-boundary GH#17671/GH#28567' "linked issue trust check carries boundary marker"
assert_not_contains ".github/workflows/linked-issue-check.yml" 'if \(/\^\(t\\d' "untrusted linked issue path has no title-only exemption"
