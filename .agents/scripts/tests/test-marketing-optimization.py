...
from marketing_attribution import AttributionRequest, _last_touch_key, build_attribution  # noqa: E402
...
from marketing_optimization_test_support import aggregate_snapshot_document  # noqa: E402
...
def experiment_event(index: int, spec: dict[str, object]) -> dict[str, object]:
    ...
    event = normalized_event("1", event_type, str(spec.get("occurred_at", "2026-08-03T12:00:00Z")), subject_id=subject_id)
...
def experiment_fixture(options: dict[str, int] | None = None) -> tuple[dict[str, object], dict[str, object], dict[str, object]]:
    ...
    for variant_id, count in variants:
        for position in range(count):
            ...
            assignments.append({"unit_ref": subject_id, "variant_id": variant_id, "assigned_at": "2026-08-01T00:00:00Z"})
            ...
            if position < settings[f"{variant_id}_conversions"]:
                ...
                index += 1
            has_positive_guardrail = (
                variant_id == "treatment" and position < settings["treatment_guardrails"]
            )
...
class MarketingOptimizationContractTests(unittest.TestCase):
    ...
    def test_live_snapshot_ignores_all_state_recorded_after_as_of(self) -> None:
        ...
        initial_epoch = int(
            datetime.fromisoformat("2026-08-11T01:00:00+00:00").timestamp()
        )
        future_epoch = int(
            datetime.fromisoformat("2026-08-12T01:00:00+00:00").timestamp()
        )
        with MarketingPerformanceStore.open(self.repo, provision=True) as store:
            with mock.patch(
                "_performance_store_leases.time.time",
                return_value=initial_epoch,
            ):
                with mock.patch(
                    "_performance_store_ingest.time.time",
                    return_value=initial_epoch,
                ):
                    with mock.patch(
                        ...
                        store.ingest(
                            ...
                            adapter_result(
                                [
                                    event("event-a", "subject-a"),
                                    event("event-b", "subject-b"),
                                ],
                                "initial-cursor",
