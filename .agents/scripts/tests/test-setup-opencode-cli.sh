...
# --- Test 5e: broken marked shim is repaired from a persistent candidate -----
echo "Test 5e: setup_opencode_cli repairs broken marked shim without install"
...
(
	...
	_setup_opencode_binary_is_ephemeral() {
		local bin="$1"
		[[ "$bin" == "$repair_bin/opencode" ]] && return 1
		return 0
	}
...
assert_eq "broken marked shim repair rc" "rc=0" "$(tail -1 "$SANDBOX/out5e")"
