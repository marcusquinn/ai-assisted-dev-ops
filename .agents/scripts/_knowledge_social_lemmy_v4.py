...
from _knowledge_social_lemmy import PageRequest, V4_STREAMS
...
def page(api: Api, request: PageRequest, _identity: dict[str, Any]) -> PageResult:
    ...
    if records and next_page is not None and next_page == request.page_cursor:
        raise LemmyReadProviderError("Lemmy v4 page cursor did not advance")
    overlap_enabled = request.stream not in {"subscriptions", "multicommunities"}
    watermark, crossed = pagination_watermark(
        records, request.watermark, overlap=overlap_enabled
    )
