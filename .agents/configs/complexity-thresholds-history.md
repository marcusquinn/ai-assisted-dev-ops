...
| 63 | GH#30674/PR #30682 (observed by GH#30684/PR #30685) | The reviewed deployment-copy state machine shipped with `ratchet-bump` and `new-file-smell-ok`, but its merge did not advance the absolute threshold. The next docs-only PR reports 61 total smells while Qlty Maintainability Smells and qlty check pass, confirming inherited accepted debt rather than a README regression. 61 actual + 2 buffer = 63; post-merge automation can ratchet down after smell-reducing merges. |
| 77 | GH#31019 | After GH#31115 advanced deterministic Qlty CLI pinning to 0.643.0, the dependency-only PR merge ref reports 75 total smells while Qlty Smell Regression, Qlty Maintainability Smells, qlty check, CodeFactor, Socket, and maintainer gates pass. The PR effective diff is only CodeQL SARIF upload and release-candidate artifact action pins, so this is accepted tool-version recount drift rather than new code debt. 75 actual + 2 buffer = 77; ratchet down after smell-reducing merges. |
