...
- When a context-rich session has established safe work and the user authorises execution, preserve momentum through verification in that session. Creating/opening its linked worktree does not require a new chat: target that path with file tools and Bash `workdir`. Defer only for a verified permission/ownership block, unrelated objective, or explicit parallel/background request.
