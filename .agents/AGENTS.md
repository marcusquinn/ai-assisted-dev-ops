...
- Interactive sessions: at an eligible safe pause after a completed PR lifecycle or a 3+ hour runtime advisory, preserve a continuation checkpoint and briefly offer `/new`; also recommend fresh context for a clearly unrelated objective. Never interrupt active work or affect headless sessions. Details: `reference/session.md`.
