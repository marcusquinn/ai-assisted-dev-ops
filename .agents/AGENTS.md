...
- With safe work and execution authority, continue through verification in the same session/worktree. Creating or opening its linked worktree does not require a new chat: target that path with file tools and Bash `workdir`. Defer only for a verified permission/ownership blocker, unrelated objective, or explicit parallel/background request. See `reference/session.md`.
