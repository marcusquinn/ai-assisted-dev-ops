...
for terminal_case in "401:authorization" "403:authorization" "404:unavailable" "500:provider"; do
	terminal_status="${terminal_case%%:*}"
	terminal_failure="${terminal_case#*:}"
	cat >"$TMP_DIR/terminal-${terminal_status}.json" <<JSON
{
  "identity": {"data": {"id": "acct42", "username": "private-handle"}},
  "pages": [{"status": ${terminal_status}}]
}
JSON
	terminal_result=$("$HELPER" sync-x --base "$BASE" --alias personal:default \
		--connection-id conn_authored --account-id acct42 --stream authored \
		--fixture "$TMP_DIR/terminal-${terminal_status}.json")
	assert_eq "${terminal_status} is classified without advancing a cursor" \
		"$(python3 -c 'import json,sys; result=json.loads(sys.argv[1]); print(result["status"] + ":" + result["failure_class"])' "$terminal_result")" \
		"failed:${terminal_failure}"
done
assert_eq "terminal provider failures preserve the checkpoint" \
	"$(sql_value "SELECT watermark FROM sync_cursors WHERE stream='authored'")" "$cursor_before"

cat >"$TMP_DIR/malformed-page.json" <<'JSON'
{
  "identity": {"data": {"id": "acct42", "username": "private-handle"}},
  "pages": [{"status": 200, "data": {"id": "not-an-array"}}]
}
JSON
batch_count=$(sql_value 'SELECT count(*) FROM fetch_batches')
if "$HELPER" sync-x --base "$BASE" --alias personal:default \
	--connection-id conn_authored --account-id acct42 --stream authored \
	--fixture "$TMP_DIR/malformed-page.json" >/dev/null 2>&1; then
	assert_eq "malformed pages fail before persistence" "accepted" "rejected"
else
	assert_eq "malformed pages fail before persistence" "rejected" "rejected"
fi
assert_eq "malformed pages preserve raw-batch state" \
