...
Load the focused references before changing the corresponding producer or
operator workflow:

- `reference/storage-lifecycle-worktree-recovery.md` — recovery archive
  evidence, bounded planning, confirmation-bound apply, automatic maintenance,
  and cache-pruning contracts.
- `reference/storage-lifecycle-store-policies.md` — npm cache monitoring,
  runtime-bundle retention and rollback, and coordinated backup/log policies.

These references extend this architecture contract. They do not grant generic
cleanup authority or weaken the ownership and safety classes above.
