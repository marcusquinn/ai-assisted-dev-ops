...
- **Release executor visibility**: status distinguishes live, dead and unknown owners from an active merge fence. `aidevops release recover-reservation SOURCE_PR` and merge guards recover only verified abandoned modern pre-publication reservations; legacy/foreign owners and possible publication remain fenced ([recovery contract](.agents/reference/release-lane-coordination.md)).
