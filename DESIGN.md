...
- The inventory is defined in `.agents/scripts/readme_inventory.py`, using tracked sources only. Main agents reuse `agent_config.SKIP_FILES` so demoted root profiles do not inflate the total. Sub agents require explicit `mode: subagent` metadata in the listed agent-source directories (plus demoted root profiles); exclude documentation-only directories, workflows, templates, imported skill fragments, tests, and aliases. Helper scripts are named production `*-helper` entry points in `.agents/scripts/` across supported script languages; supporting implementation modules are not additional helpers. Slash commands are direct Markdown entry points in `scripts/commands`, including aliases only when their targets are tracked regular files inside `.agents`.
...
- Audit with `bash .agents/scripts/readme-helper.sh counts --inventory` (counts, definitions, and every included path). `counts --json` gives exact totals; `counts --approx` gives hero figures. The Git index selects paths; metadata comes from their current worktree contents so pending profile edits are reflected. Stage new/deleted source paths before counting; untracked/deployed/custom files are excluded. `check` validates every matching README claim and all four structured SVG figures plus metadata. `update --apply` refreshes both text sources from the same inventory; it does not render or verify the PNG, which requires the visual step below.
